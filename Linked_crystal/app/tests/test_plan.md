...
- **Expected Result**: 
    - [x] `to_bytes()` returns a 24-byte buffer.
    - [x] `from_bytes(buffer)` recreates an identical object.
...
- **Expected Result**: 
    - [x] `PacketDispatcher.my_id` becomes `5`.
    - [x] `AppData.userID` is updated.
...
- **Expected Result**: [x] `ChatManager.receive_message` is called with correct ID and text.
...
- **Expected Result**: 
    - Link stats dot turns Green ("Linked!"). (Backend Verified in `test_link_scenarios.py`)
...
- **Expected Result**: [x] Client automatically establishes connection within `backoff` window. (Verified in `test_reconnection.py`)
...
- **Expected Result**: [x] Queue size remains at 10,000; oldest bytes are dropped; no crash.
